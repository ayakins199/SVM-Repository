...
namespace SVM
{
    ...
    public class PerformanceEvaluator
    {
        ...
        /// <param name="correctLabels">The correct labels of each data item</param>
        /// <param name="problem">Problem to evaluate</param>
        /// <param name="TP">True Positive</param>
        /// <param name="TN">True Negative</param>
        /// <param name="FP">False Positive</param>
        /// <param name="FN">False Negative</param>
        /// <param name="P">Precision</param>
        /// <param name="F_M">F-Measure</param>
        public PerformanceEvaluator(string resultsFile, Problem problem, out double TP, out double TN, out double FP, out double FN,
                                    out double P, out double F_M)
        {
            FP = 0.0; FN = 0.0; TP = 0.0; TN = 0.0; P = 0.0; F_M = 0.0;
            int fpCntr = 0, fnCntr = 0, tpCntr = 0, tnCntr = 0;
            ...
            {
                if (problem.Y[i] == +1.0 && double.Parse(parts[i], CultureInfo.InvariantCulture) == +1.0)
                    tpCntr++;
                else if (problem.Y[i] == +1.0 && double.Parse(parts[i], CultureInfo.InvariantCulture) == -1.0)
                    fnCntr++;
                else if (problem.Y[i] == -1.0 && double.Parse(parts[i], CultureInfo.InvariantCulture) == -1.0)
                    tnCntr++;
                else if (problem.Y[i] == -1.0 && double.Parse(parts[i], CultureInfo.InvariantCulture) == +1.0)
            ...
            FP = (fpCntr > 0) ? (fpCntr / (double)(fpCntr + tnCntr)) : 0;
            FN = (fnCntr > 0) ? (fnCntr / (double)(fnCntr + tpCntr)) : 0;
            TP = (tpCntr > 0) ? (tpCntr / (double)(tpCntr + fnCntr)) : 0;
            TN = (tnCntr > 0) ? (tnCntr / (double)(tnCntr + fpCntr)) : 0;
            P = (tpCntr > 0) ? (tpCntr / (double)(tpCntr + fpCntr)) : 0;
            F_M = 2 * P * TP / (P + TP);
