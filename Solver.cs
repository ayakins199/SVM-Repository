    ...
    internal class Procedures
    {
            ...
            else
            {
                ...
                for (i = 0; i < nr_class; i++)
                    for (int j = i + 1; j < nr_class; j++)
                    {
                        ...
                        for (k = 0; k < ci; k++)
                        {
                            ...
                            {
                                nonzero[si + k] = true; //ctr++; if (ctr > 25) break;
                            }
                        }
                        //ctr = 0;
                        for (k = 0; k < cj; k++)
                        {
                            ...
                            {
                                nonzero[sj + k] = true; //if (ctr > 25) break; ctr++;
                            }
                        }
                        ++p;
                    }
                ...
                for (i = 0; i < l; i++)
                    if (nonzero[i])
                    {
                        ...
                        model.SupportVectors[p] = x[i];
                        model.SupportVectorsClassLabels[p] = y[i];
                        p++;
                    }
        ...
        public static void svm_cross_validation(Problem prob, Parameter param, int nr_fold, double[] target)
        {
            ...
            for (i = 0; i < nr_fold; i++)
            {
                    ...
                    param.SvmType == SvmType.NU_SVC))
                {
                    ...
                    for (j = begin; j < end; j++)
                    {
                        File.AppendAllText(ps.filePath2, prob.Y[perm[j]].ToString() + "  ");
        ...
        public static void svm_predict_values(Model model, Node[] x, double[] dec_values)
        {
            ...
            else
            {
                ...
                //model.NumberOfSVPerClass[0] = tot / 4;
                //model.NumberOfSVPerClass[1] = tot / 4;
                //int a = 0;
                ...
                for (i = 0; i < nr_class; i++)
                    for (int j = i + 1; j < nr_class; j++)
                    {
                        ...
                        for (k = 0; k < ci; k++)
                        {
                            //sum += coef1[str[ss]] * kvalue[ss];
                            //ss++;
                            sum += coef1[si + k] * kvalue[si + k];
                        }
                        for (k = 0; k < cj; k++)
                        {
                            //sum += coef2[str[ss]] * kvalue[ss];
                            //ss++;
        ...
        public static double svm_predict(Model model, Node[] x)
        {
            ...
            else
            {
                ...
                outputValues = dec_values[0].ToString() + "  "  +  model.ClassLabels[vote_Max_idx].ToString();
                File.AppendAllText(ps.filePath2, outputValues);
                File.AppendAllText(ps.filePath2, Environment.NewLine);
        ...
        public static Model reduceSupportVector(Model model, Problem problem)
        {
