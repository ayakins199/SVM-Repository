...
namespace ClusteringKMeans
{
  public class KMeansDemo
  {
    ...
    //public static double[][] extractData(List<Node[]> rawData)
    public static double[][] extractData(List<object[,]> rawData)
    { 
        ...
        for (int i = 0; i < rawData.Count; i++)
        {
            ...
            vector = (Node[])rawData[i][0, 0];
            for (int j = 0; j < vecCount; j++)
            {
                //clusData[0][j] = rawData[i][j].Value;
                clusData[0][j] = vector[j].Value;
            }
    ...
    //public static double[][] Cluster(List<Node[]> rawData, int numClusters)  
    public static double[][] Cluster(List<object[,]> rawData, int numClusters)  
    {
      // k-means clustering
      ...
      double[][] dataExtract = extractData(rawData);
      double[][] data = Normalized(dataExtract); // so large values don't dominate
    ...
    private static int[] InitClustering(int numTuples, int numClusters, int randomSeed)
    {
      ...
      numTuples = numTuples < numClusters ? numClusters : numTuples; //ensuring that numTuples is not less than numClusters, to avoid "index out of range" error
